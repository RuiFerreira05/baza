...
import { createOrEditEventPreference, getEventPreferenceByUsername, getEventPreferences, getGroupPreferenceAggregation } from "../services/preferenceServices";
import { createStatusError, createStatusOK, ErrorTypes, type CreatePreferenceBody } from "@baza/shared-types";
import { app } from "../setup";

// POST /groups/:id/events/:idevent/preferences/create (or edit)
export const createOrEditEventPreferenceHandler = async (req: FastifyRequest, res: FastifyReply) => {
  ...
  const result = await createOrEditEventPreference(groupId, eventId, username, body);
...
export const getEventPreferenceByUsernameHandler = async (req: FastifyRequest, res: FastifyReply) => {
  ...
  const { id: groupId, idevent: eventId, username } = req.params as { id: string; idevent: string; username: string };

  const result = await getEventPreferenceByUsername(groupId, eventId, username, requesterUsername);
...
export const getEventPreferencesHandler = async (req: FastifyRequest, res: FastifyReply) => {
...
export const getGroupPreferenceAggregationHandler = async (req: FastifyRequest, res: FastifyReply) => {
