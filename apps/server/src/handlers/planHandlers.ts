...
import { createEventPlan, getEventPlans, getEventPlanById, editEventPlan, voteEventPlan, removeVoteEventPlan } from "../services/planServices";
import { createStatusError, createStatusOK, ErrorTypes, type CreatePlanBody, type EditPlanBody } from "@baza/shared-types";
import { app } from "../setup";

// POST /groups/:id/events/:idevent/plans/create
export const createEventPlanHandler = async (req: FastifyRequest, res: FastifyReply) => {
...
export const getEventPlansHandler = async (req: FastifyRequest, res: FastifyReply) => {
...
export const getEventPlanByIdHandler = async (req: FastifyRequest, res: FastifyReply) => {
  app.log.info("Received Get Event Plan by ID request");
  const username = await getAuthenticatedUsername(req, res);
  if (!username) return;

  const { id: groupId, idevent: eventId, idplan: planId } = req.params as { id: string; idevent: string; idplan: string };
...
export const editEventPlanHandler = async (req: FastifyRequest, res: FastifyReply) => {
...
export const voteEventPlanHandler = async (req: FastifyRequest, res: FastifyReply) => {
  ...
  if (!result.ok) {
    ...
    return res.status(200).send(createStatusOK({ message: "Vote successfully casted." }));
  }
};

// POST /groups/:id/events/:idevent/plans/:idplan/remove-vote
export const removeVoteEventPlanHandler = async (req: FastifyRequest, res: FastifyReply) => {
  ...
  if (!result.ok) {
    ...
    return res.status(200).send(createStatusOK({ message: "Vote successfully removed." }));
  }
};
