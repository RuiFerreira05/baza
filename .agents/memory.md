...
* **Maps & Places Autocomplete**: Location inputs in event scheduling use the custom `GooglePlacesMapInput` component. It uses Google Places API for autocomplete suggestion and coordinates fetching, `expo-maps` for native interactive map display (Apple Maps on iOS, Google Maps on Android), and Google Geocoding API for reverse-lookup on map pin placement. Mapped locations are saved to the database as serialized JSON strings containing coordinates and names, while manually-typed plain text strings are supported with a fallback layout. Display components parse and format these values using the `formatLocation` utility (in `src/lib/location.ts`) to show only the readable name.
...
21. **Event Preferences Date Selection & Spacing**: The day preference in MyPreferencesForm uses the Wix `<Calendar>` component with minDate and maxDate bound to the event details dates. If dates do not span multiple months, month navigation (arrows and swipe) is disabled and hidden using `hideArrows` and `disableMonthChange`. The activities input field and button align cleanly without overflow by using centeraligned flex layouts in `useEventPreferencesStyles.ts` and a shorter placeholder text.
22. **Group View Analytics & Dashboard Resolution**: The Group View dashboard uses a client-side ViewModel mapper to align the server's basic `GroupPreferenceReportDTO` response with the dashboard's analytics properties. It retrieves group members count dynamically using `membersQuery` to populate the `totalGroupMembers` and `submittedCount` header properly (preventing the empty slash header bug), and aggregates extra fields (duration, environment, group size average, time of day) on the client side from `allPreferences`. Every preference field supports an explicit "No preference" (or "Optional" leave blank) option, with custom grid layout (e.g. 2x2/2x2+1) in MyPreferencesForm to prevent label wordwrapping (such as for the "afternoon" label). Both the Time of Day and Preferred Duration preferences support multiple selection (stored as string arrays), with backward-compatible aggregation logic that supports legacy single-string data structures.
